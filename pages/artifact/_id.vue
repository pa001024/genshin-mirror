...
import type { IArtifactType } from "~/modules/core";
...
@Component<Page>({
  ...
  async asyncData({ params: { id }, $content, app }) {
    ...
    const res = (await $content(app.i18n.locale, "relic").where({ id }).fetch().catch(console.error)) as any;
    rst.data = res && res[0];
