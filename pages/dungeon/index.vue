      ...
      <v-list>
        <template v-for="(item, index) in items">
          ...
          <v-list-item v-if="item.id" :key="item.id">
            <div class="dungeon-linehead">
              <v-list-item-title>
                <v-avatar color="grey" />
              </v-list-item-title>
            </div>

            <nuxt-link :to="'dungeon/' + item.id" class="nolink">
              <v-list-item-content>
...
@Component<Page>({
  ...
  head() {
    ...
    const title = this.$t("title.sub", [this.$t("title.dungeon")]) as string;
    return { title };
  },
})
export default class Page extends Vue {
  ...
  get types(): FilterOption[] {
    return [...new Set(this.data?.map(v => v.type) || [])].map(v => ({ text: this.$t("dungeon." + v) as string, prop: "type", value: v }));
  }
    ...
    return this.data
      ...
      .reduce((r, item: any, index) => {
        if (index === 0 || r[index - 1].type !== item.type) {
          item = { ...item, subtitle: this.$t(`dungeon.${item.type}`) as string };
        }
        r.push(item);
        return r;
      }, [] as Partial<IDungeon & { subtitle: string }>[]);
  }
}
</script>
